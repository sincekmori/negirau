...
- **Reaction counters are day-granular** (`reaction_counts` PK subject/type/day) to allow surgical rollback of attacked days. All-time is the only aggregation any surface shows, which makes collapsing the table into one row per subject look tempting — do not: the day column is the moderation lever, not a display feature.
