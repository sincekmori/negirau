...
name: negirau
description: >-
  Work with Negirau (negirau.com), the login-free one-tap appreciation service for the people behind everyday work — fire stations, event crews, online services, open-source maintainers.
  Use this whenever a task touches Negirau in any way: reading subjects or reaction display values from the public API, embedding a badge or an Atom feed, linking or printing a thank-you page, creating a new subject, or explaining why exact counts above 100, rankings, and API-driven sends do not exist (deliberate design, not gaps).
...
- `{ORIGIN}` — the page origin.
  Default: `https://negirau.com`
- `{API}` — the read-API base.
  Default: `https://api.negirau.com/v1`
...
- **Counts are capped.**
  Every public surface shows a count exactly up to 100 and as `"100+"` beyond — that is the only boundary, and unbounded exact numbers never leave the server.
- **No rankings.**
...
```
curl "{API}/subjects?q=library&limit=5"
```

| Operation                      | What it does             |
| ------------------------------ | ------------------------ |
| `GET /subjects`                | List and search subjects |
| `GET /subjects/{id}`           | Get one subject          |
| `GET /subjects/{id}/reactions` | Reaction display values  |
...
- The page offers a 5-emoji reaction palette (❤️ 👍 🤝 🌸 🍵) — deliberately captionless, so the feeling stays the sender's own; one tap sends, tapping the pressed emoji again within about a minute takes it back, and each device can send each emoji once per subject per day.
...
They default to `https://api.negirau.com/v1`; for another instance pass `{API}` explicitly (`baseURL` in TypeScript, `base_url` in Python, the `NEGIRAU_BASE_URL` env var for the MCP server).
...
console.log(reactions.total); // a display value like "100+"
```
